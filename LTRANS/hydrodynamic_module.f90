...
CONTAINS
  ...
  SUBROUTINE initGrid()
    ...
    if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      ...
      !write(*,*)'shape of mask_u:',shape(mask_u)
      ! mask on v grid
...
!    write(*,*) TRIM(filenm)
    ...
    m_u = mask_u
    m_v = mask_v
    !write(*,*)'m_rho',mask_u(530,751), mask_u(531,751)
    !write(*,*)'m_u',m_u(530,751), shape(m_u)
    ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  SUBROUTINE initHydro()
    ...
    ALLOCATE(romKH(vi,uj,ws,3))
      ...
      END SELECT
      write(*,*) TRIM(filenm)
      OPEN(310,FILE='Log_jx.txt',POSITION='APPEND')
       write(310,*) TRIM(filenm)
       close(310)
...
!       write(*,*) 'ncid = ', ncid
      ...
      write(*,*)t_ijruv(5),t_ijruv(6),t_ijruv(7),t_ijruv(8),ui
      do j=t_ijruv(7),t_ijruv(8)
        do i=t_ijruv(5),t_ijruv(6)
          ...
          !  t_Uvel(3,count,k) = romU(i,j,k,3) * m_u(i,j)
             if(m_u(i,j) .EQ. 0.)  then
               t_Uvel(1,count,k) = 0.0
               t_Uvel(2,count,k) = 0.0
               t_Uvel(3,count,k) = 0.0  !jx: another way to force land value to be 0
             else
               t_Uvel(1,count,k) = romU(i,j,k,1)
               t_Uvel(2,count,k) = romU(i,j,k,2)
               t_Uvel(3,count,k) = romU(i,j,k,3)
             endif
    !        write(*,*)t_Uvel(1,count,k),romU(i,j,k,1),m_u(i,j),i,j,k
          enddo
    ! write(*,*)t_Uvel(1,count,k),romU(i,j,k,1),m_u(i,j)
      ...
      do j=t_ijruv(11),t_ijruv(12)
        ...
        !    t_Vvel(3,count,k) = romV(i,j,k,3) * m_v(i,j)
            if(m_v(i,j).EQ.0.)  then
              t_Vvel(1,count,k) = 0.0
              t_Vvel(2,count,k) = 0.0
              t_Vvel(3,count,k) = 0.0  !jx: another way to force land value to be 0
            else
              t_Vvel(1,count,k) = romV(i,j,k,1)
              t_Vvel(2,count,k) = romV(i,j,k,2)
              t_Vvel(3,count,k) = romV(i,j,k,3)
            endif
          enddo
        enddo    
      enddo
...
!   write(*,*) 'ncid = ', ncid
    ...
    do j=t_ijruv(3),t_ijruv(4)
      do i=t_ijruv(1),t_ijruv(2)
        ...
        if(m_r(i,j) .EQ. 0) then
          t_salt(t_f,count,k) = 0.
          t_temp(t_f,count,k) = 0.
          t_Wvel(t_f,count,k) = 0.
          t_den(t_f,count,k)  = 0.
          t_KH(t_f,count,k)   = 0.
        else
  ...
  enddo
    ...
    do j=t_ijruv(7),t_ijruv(8)
      do i=t_ijruv(5),t_ijruv(6)
        ...
        do k=1,us    
         ...
         if(m_u(i,j).EQ.0.) then
           t_Uvel(t_f,count,k) = 0.0 ! jx another way to force land value to be 0
         else
    ...
    OPEN(310,FILE='Log_jx.txt',POSITION='APPEND')
    write(310,*) 'existing matrix,stepf=',stepf
   close(310)
  ...
  DOUBLE PRECISION FUNCTION interp(xp,yp,var,i)
    ...
    SELECT CASE(var)
      ...
      CASE("uvelb")
        ...
        RUV = 2
       ! write(*,*)'v1,v2,v3,v4= ',v1,v2,v3,v4,t_b,i  !jx
      CASE("uvelc")
        if(.not. present(i))then
          write(*,*) 'Problem interpolating ',var
          write(*,*) 'Optional Argument (i) Required for this Variable'
         ...
         !Ensure there is no friction near land (the free slip condition) ewn.v.2
          IF (FreeSlip) THEN 
            ...
            umasksum = u_mask(unode1)+ u_mask(unode2)+ u_mask(unode3)          &
                     + u_mask(unode4)
            !if (umasksum .EQ. 3) write(*,*)"jx-umasksum",umasksum
            ! rmasksum – integer - the sum of the mask values of each node
            if (umasksum .LT. 4) then  
              ! determine how many land nodes are present and reassign values 
              if (umasksum .EQ. 3) then   !one land node
  ...
  DOUBLE PRECISION FUNCTION WCTS_ITPI(var,Xpos,Ypos,deplvl,Pwc_zb,Pwc_zc,      &
    ...
    do i=1,nN
     ...
     ! write(*,*)'abb_vb= ',abb_vb(i), Xpos, Ypos, varb, i+deplvl-1, i
      abb_vc(i) = interp(Xpos,Ypos,varc,i+deplvl-1)
     ! write(*,*)'abb_vc= ',abb_vc(i), Xpos, Ypos, varc, i+deplvl-1, i
      abb_vf(i) = interp(Xpos,Ypos,varf,i+deplvl-1)
     ! write(*,*)'abb_vf= ',abb_vf(i), Xpos, Ypos, varf, i+deplvl-1, i
    enddo
...
!write(*,*)'WCTS_ITPI-jx1',nN,abb_zb,abb_vb,YP,SIGM,IER,SigErr
