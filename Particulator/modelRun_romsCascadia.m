	...
	methods
		...
		function run = modelRun_romsCascadia(arg1,arg2);
			...
			%	fsub = find([dirlisting.isdir] & names(:,1)~='.');
				...
				run.filename = {};
				for i=1:length(subdirs)  
					thefiles = dir([dirname subdirs{i} '/ocean_his_*.nc']);
                    temp = {thefiles.name}; %jx, not read ocean_his_0025.nc to avoid duplication
                    subfilenames = {temp{1:24}}; % jx
					% subfilenames = {thefiles.name}; %jx
					if isempty(subfilenames)
						warning(['no files found at ' dirname subdirs{i} '/ocean_his_*.nc']);
					end			
					for j=1:length(subfilenames)
						run.filename = cat(1,run.filename,[dirname subdirs{i} '/' subfilenames{j}]);
					end
				end
                ...
                temp = {thefiles.name}; %jx, not read ocean_his_0025.nc to avoid duplication
                run.filename = {temp{1:24}}; %jx
				%run.filename = {thefiles.name}; %jx
			...
			%jx  timeref = datenum(timeref,'yyyy-mm-dd HH:MM:SS');
            timeref = datenum(timeref);
